---
permalink: /user-guide/reference/studio-interface/dialogs/layer-token/
...
ui:
  ...
  overview: >
    The Layer token modal appears when a protected layer or terrain source needs an access token. It keeps the token request tied to the source that needs it.
  controls:
    - label: Token field
      icon: key
      type: Text field
